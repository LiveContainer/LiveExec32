...
OUTPUT=${1:-/private/tmp/lc32-coretext-font-descriptor}
OBJECT=${OUTPUT}.o
...
clang -arch armv7s -isysroot "$SDKROOT" -miphoneos-version-min=10.3 \
    -c "$SCRIPT_DIR/coretext_font_descriptor.c" -o "$OBJECT"
clang -arch armv7s -isysroot "$SDKROOT" -miphoneos-version-min=10.3 \
    "$OBJECT" -F"$FRAMEWORK_DIR" \
    -framework CoreText -framework CoreFoundation -framework Foundation \
