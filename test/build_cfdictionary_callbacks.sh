...
OUTPUT=${1:-/private/tmp/lc32-cfdictionary-callbacks}
OBJECT=${OUTPUT}.o
...
clang -arch armv7s -isysroot "$SDKROOT" -miphoneos-version-min=10.3 \
    -c "$SCRIPT_DIR/cfdictionary_callbacks.m" -o "$OBJECT"
clang -arch armv7s -isysroot "$SDKROOT" -miphoneos-version-min=10.3 \
    "$OBJECT" -F"$FRAMEWORK_DIR" \
    -framework Foundation -framework CoreFoundation -framework LC32 \
