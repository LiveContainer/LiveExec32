...
OUTPUT=${1:-/private/tmp/lc32-security-secitem}
OBJECT=${OUTPUT}.o
...
clang -arch armv7s -isysroot "$SDKROOT" -miphoneos-version-min=10.3 \
    ...
    -c "$SCRIPT_DIR/security_secitem.m" -o "$OBJECT"
clang -arch armv7s -isysroot "$SDKROOT" -miphoneos-version-min=10.3 \
    "$OBJECT" -F"$FRAMEWORK_DIR" \
    -framework Foundation -framework Security -framework LC32 \
    -fno-autolink -o "$OUTPUT"

file "$OUTPUT"
otool -L "$OUTPUT"
"$SCRIPT_DIR/security_symbols.sh" \
    "$FRAMEWORK_DIR/Security.framework/Security"
