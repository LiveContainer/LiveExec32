...
OUTPUT=${1:-/private/tmp/lc32-cxx-lifecycle}
OBJECT=${OUTPUT}.o
FRAMEWORK_DIR="$REPO_ROOT/GuestMakefile/.theos/obj/debug/armv7s"

clang++ -x objective-c++ -std=c++14 -arch armv7s -isysroot "$SDKROOT" \
    -miphoneos-version-min=10.3 \
    -c "$SCRIPT_DIR/cxx_lifecycle.mm" -o "$OBJECT"
clang++ -arch armv7s -isysroot "$SDKROOT" \
    -miphoneos-version-min=10.3 \
    "$OBJECT" -F"$FRAMEWORK_DIR" \
    -framework Foundation -framework UIKit -framework LC32 \
    -fno-autolink \
    -o "$OUTPUT"
