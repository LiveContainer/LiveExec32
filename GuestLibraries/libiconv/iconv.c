/*
 * Guest libiconv conversion core.
 *
 * Based on musl's iconv implementation (src/locale/iconv.c), used here under
 * the MIT license; see LICENSE in this directory. Modifications for
 * LiveExec32:
 * - dropped the musl locale machinery: the UTF-8 steps use the local
 *   stateless UTF-8 decoders below instead of mbrtowc/wctomb, so conversion
 *   results never depend on the guest's active locale,
 * - added an identity mapping for Latin-1 and its common aliases,
 * - added iconvctl for the GNU/Apple libiconv control API.
 */
...
"latin1\0iso88591\0iso_8859_1\0cp819\0l1\0\0\100"
#include "codepages.h"
;
...
/*
 * Self-contained UTF-8 decoders so conversions never depend on the guest's
 * active locale. The mbstate argument is accepted for the musl call shape but
 * is unused: the decoder is stateless and *in is never advanced on partial
 * input, so every starved sequence is simply re-decoded from its first byte.
 */
static size_t mbrtowc_utf8(wchar_t *restrict wc,
	const char *restrict src, size_t len, mbstate_t *restrict st)
{
	(void)st;
	if (!len) return (size_t)-2;
	unsigned char c = (unsigned char)src[0];
	if (c < 0x80) {
		*wc = c;
		return 1;
	}
	unsigned need, value, minimum;
	if ((c & 0xE0) == 0xC0) {
	...
	if (len < (size_t)need + 1) return (size_t)-2;
	for (unsigned i = 1; i <= need; i++) {
		unsigned char continuation = (unsigned char)src[i];
		if ((continuation & 0xC0) != 0x80) goto invalid;
		value = value << 6 | (continuation & 0x3F);
	}
	if (value < minimum || value > 0x10FFFF) goto invalid;
	if ((unsigned)(value - 0xD800) < 0x800) goto invalid;
	*wc = value;
...
static int wctomb_utf8(char *dest, wchar_t wc)
{
	...
	if (c < 0x10000) {
		if ((unsigned)(c - 0xD800) < 0x800) return -1;
		dest[0] = 0xE0 | c >> 12;
...
/*
 * GNU/Apple libiconv control API. Transliteration, discarded-illegal-sequence
 * and hook/fallback requests are accepted and ignored: the core always
 * reports invalid input sequences through EILSEQ and never substitutes.
 */
int iconvctl(iconv_t cd, int request, void *argument)
{
	...
	switch (request) {
	case 0: /* ICONV_TRIVIALP */
		*(int *)argument = extract_to(cd) == extract_from(cd);
		return 0;
	case 1: /* ICONV_GET_TRANSLITERATE */
		*(int *)argument = 0;
		return 0;
	case 2: /* ICONV_SET_TRANSLITERATE */
	case 3: /* ICONV_GET_DISCARD_ILSEQ */
		*(int *)argument = 0;
		return 0;
	case 4: /* ICONV_SET_DISCARD_ILSEQ */
	case 5: /* ICONV_SET_HOOKS */
	case 6: /* ICONV_SET_FALLBACKS */
		return 0;
	default:
		errno = EINVAL;
		return -1;
